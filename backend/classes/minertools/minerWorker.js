...
class MinerWorker{
    ...
    async stop(){
        ...
        this.send({ isStopped:true })
    }

    async start(rawBlock){
        this.send({isPreparing:true})
        let block = await this.prepareBlockForMining(rawBlock);
        if(block){
            this.send({isMining:true})
            ...
            if(success){
                ...
                this.send({ success:block })

            }else{
                this.log('Mining failed')
                this.send({ failed:true })
            }
        }
    }
...
let miner = new MinerWorker({
    send:(...message)=>{
        parentPort.postMessage(...message)
    },
    keychain:workerData.keychain,
    clusterMiner:workerData.clusterMiner,
    verbose:workerData.verbose
})
miner.initWallet()
.then( walletOn => {
    parentPort.on('message',async (message)=>{
        if(message.previousBlock){
            miner.previousBlock = message.previousBlock
        }
        else if(message.mine){
            await miner.start(message.mine)
        }
