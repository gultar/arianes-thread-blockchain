...
class StateStorage{
    ...
    async update(state){
        if(state && Object.keys(state).length > 0 && !state.error){
            let blockNumber = this.getCurrentBlock().blockNumber + 1 //because the newest block is still being merged with the chain
    ...
    async parseBlockNumbers(keys){
        try{
            ...
            let sortedBlockNumbers = await blockNumbers.sort((a,b)=>b-a)
    ...
    async sortBlockNumbers(keys){
        let highestBlockNumber = 0

        for await(let key of keys){
            if(parseInt(key) > parseInt(highestBlockNumber)){
    ...
    async orderByTime(){
        let ordered = await this.database.database.get('TokensStorage','*', require('rocket-store').ORDERBY_TIME)
        console.log(ordered)
    }
