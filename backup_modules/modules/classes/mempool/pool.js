...
class Mempool{
    ...
    deferPayable(transaction){
        if(transaction && transaction.delayToBlock){
            //Add extra validation
            this.delayedTransactions[transaction.hash] = transaction
    ...
    getTransactionHashes(){
        return new Promise(async (resolve)=>{
            let keys = await this.transactions.getAllKeys()
            if(keys){
                if(keys.error) resolve({error:keys.error})
                else resolve(keys)
            }else{
                resolve([])
            }
    ...
    getActionHashes(){
        return new Promise(async (resolve)=>{
            let keys = await this.actions.getAllKeys()
            if(keys){
                if(keys.error) resolve({error:keys.error})
                else resolve(keys)
            }else{
